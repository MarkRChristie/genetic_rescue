Selection <- function(pops, parameters) {
  ...
  probs      <- (m*phenotypes) + b  #NOTE THAT THIS IS SOMETIMES INDENTICAL TO PHENPTYPES
  ...
  #test.phenotypes <- seq(from = 0, to = 2, by = 0.01) # 2 = optimum in this example; 3 lines illustrate current selection intensity
  #selection.strength2 <- (2-(test.phenotypes))/2 # no mean here because assuming mean 
  #plot(test.phenotypes, selection.strength2) # linear decrease in selection strength

  if(selection.strength > 0.5) {selection.strength = 0.5} # despite above, cap selection at 0.5 at any point in time 
  if(selection.strength <= 0) {selection.strength = 0.01} # always maintian slight selection?
